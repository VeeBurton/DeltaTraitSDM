...
rsmax3 <- stack(ST_H, MWMT_P, PAS_T) 
names(rsmax3)= c("Hraw","MWMT_P","PAS_T")
names(rsmax3)
plot(rsmax3)

pair.mod2 <- lmer(log(W17Height) ~ scale(MWMT_P) + scale(PAS_T) + scale(MWMT_P)*scale(PAS_T) + (1|Trial/Block/Provenance), data=sp.raw)
predH3 <- predict(rsmax3, pair.mod2, re.form=NA, type='response') #whittet.mod2 #dredge1
predH3<- exp(predH3)
predH3_crop <- crop(predH3, scot)
plot(predH3_crop, main="Tree height predicted (pair.mod2)")
