...
#predict height
#whittet.mod1 <- lmer(log(Hraw) ~ MAP_T + FFP_P + MAP_T*FFP_P + (1|Trial/Block/Provenance),data=sp)
#predH <- predict(rsmax, whittet.mod1, re.form=NA, type='response') 
#predH<- exp(predH)
...
# esc baseline
esc <- raster(paste0(wd,"Scots_pine/sp_esc_yc_gb_cont_0_7_min.tif"))
plot(esc)
crs(esc)
crs(esc)<- "+init=epsg:27700" # british national grid
# mask using esc
predDist<- mask(predH, esc)
plot(predDist)
