...
sp.raw %>% 
  #mutate(nest=factor(Trial:Block:Provenance)) %>%
  ggplot(aes(Provenance,W17Height,colour=Trial))+geom_boxplot()+
  theme_minimal()+
  ylab("Height (mm)")+
...
compare.mods <- read.csv(paste0(wd,'/data-raw/compare_performance_interactions2.csv'))
compare.mods$X<-NULL
...
results<-read.csv(paste0(wd,'data-raw/cross-validation_results_interactions2.csv'))
#summary(results)
...
climate <- read.csv(paste0(wd,"data-raw/for-climateEU_Normal_1961_1990Y.csv"))
#head(climate)
colnames(climate)<-c('x','y','lat','long','elev','MAT','MWMT','MCMT','TD','MAP','MSP','AHM','SHM','DD0','DD5','DD_18','DD18','NFFD','bFFP','eFFP','FFP','PAS','EMT','Eref','CMD')
climate[climate=="-9999"]<-NA
climate <- na.omit(climate)
...
UK <- shapefile(paste0(wd,"data-raw/european_region_region.shp"))
#unique(UK$NAME)
...
clim07 <- read.csv(paste0(wd,"data-raw/forclimateEU_Year_2007MSY.csv"))
#head(clim07)
...
clim12 <- read.csv(paste0(wd,"data-raw/forclimateEU_Year_2012MSY.csv"))
#head(clim12)
...
nwss <- shapefile(paste0(wd,"data-raw/FCPRODUCT_S_NWSS.shp")) 
#summary(nwss)
nwss<-st_as_sf(nwss)
