...
ggboxplot(
  ...
  color = "Provenance", palette = "ucscgb")

```

Outliers in the data were removed, and checks for normality carried out. As the data is not normal, a Kruskal Wallis test was used to check for significant differences between the mean height of each provenance within each trial.
This showed that there are statistically significant differences between provenances and trials (effect size 0.25, p<0.0001), illustrating thate there is both intra-specific genetic variability and phenotypic plasticity in the sample.
...
# initial models
pair.mod1 <- lmer(height ~ scale(MWMT_T) + scale(PAS_P) + scale(MWMT_T)*scale(PAS_P) + (1|Trial/Block/Provenance), data=sp.raw)
pair.mod2 <- lmer(log(height) ~ scale(MWMT_P) + scale(PAS_T) + scale(MWMT_P)*scale(PAS_T) + (1|Trial/Block/Provenance), data=sp.raw)
pair.mod3 <- lmer(log(height) ~ scale(TD_P) + scale(Eref_T) + scale(TD_P)*scale(Eref_T) + (1|Trial/Block/Provenance), data=sp.raw)
pca.mod1 <- lmer(log(height) ~ scale(DD_18_T) + scale(FFP_P) + scale(DD_18_T)*scale(FFP_P) + (1|Trial/Block/Provenance), data=sp.raw)
pca.mod2 <- lmer(log(height) ~ scale(DD_18_T) + scale(TD_P) + scale(DD_18_T)*scale(TD_P) + (1|Trial/Block/Provenance), data=sp.raw)
pca.mod3 <- lmer(log(height) ~ scale(FFP_T) + scale(TD_P) + scale(FFP_T)*scale(TD_P) + (1|Trial/Block/Provenance), data=sp.raw)
diff.mod1 <- lmer(log(height) ~ scale(PAS_P) + TD_T + scale(PAS_P)*TD_T + (1|Trial/Block/Provenance), data=sp.raw)
diff.mod2 <- lmer(log(height) ~ scale(MWMT_P) + scale(MCMT_T) + scale(MWMT_P)*scale(MCMT_T) + (1|Trial/Block/Provenance), data=sp.raw)
diff.mod3 <- lmer(log(height) ~ scale(DD0_P) + scale(PAS_T) + scale(DD0_P)*scale(PAS_T) + (1|Trial/Block/Provenance), data=sp.raw)
# "best models" from dredge )
dredge1<-lmer(log(height) ~ scale(DD_18_T) + scale(NFFD_P) + scale(DD_18_T)*NFFD_P + (1|Trial/Block/Provenance),data = sp.raw)
...
whittet.mod1 <- lmer(log(height) ~ scale(MAP_T) + scale(FFP_P) + scale(MAP_T)*scale(FFP_P) + (1|Trial/Block/Provenance), data = sp.raw)
whittet.mod2 <- lmer(log(height) ~ scale(MAP_P) + scale(FFP_T) + scale(MAP_P)*scale(FFP_T) + (1|Trial/Block/Provenance), data = sp.raw)
whittet.mod3 <- lmer(log(height) ~ scale(DD5_T) + scale(MWMT_P) + scale(MCMT_T) + scale(MAP_T) + scale(TD_P) + scale(DD5_T)*scale(MWMT_P) + (1|Trial/Block/Provenance), data = sp.raw)
...
#compare.mods <- read.csv(paste0(wd,'/Scots_pine/compare_performance_interactions2.csv')) # PC
#compare.mods <- read.csv(paste0(wd,'/data-raw/compare_performance_interactions2.csv')) # mac
#compare.mods$X<-NULL
...
This involves splitting the dataset into ‘training’ and ‘testing’ samples, fitting each model repeatedly (1000 iterations), and recording the mean absolute error (MAE) and R2 marginal from each run.
The results of the cross validation (Figure \@ref(fig:cross-validation)) indicate that "dredge2" and "dredge3" have the highest R2 marginal. Both also showed the lowest MAE.
...
for(i in 1:1000) {
  ...
  #i<-1
  rows<-sample(x=1:nrow(sp.raw),size=round(nrow(sp.raw)*0.25,1))
  ...
  dredge3<-lmer(log(height) ~ scale(DD_18_T) + scale(NFFD_P) + scale(TD_T) + scale(DD_18_T)*scale(NFFD_P) + scale(NFFD_P)*scale(TD_T)+ (1|Trial/Block/Provenance),data = training)
  dredge2<-lmer(log(height) ~ scale(DD_18_T) + scale(NFFD_P) + scale(PAS_T) + scale(DD_18_T)*scale(NFFD_P) + scale(NFFD_P)*scale(PAS_T) + (1|Trial/Block/Provenance),data = training)
  whittet.mod3 <- lmer(log(height) ~ scale(DD5_T) + scale(MWMT_P) + scale(MCMT_T) + scale(MAP_T) + scale(TD_P) + scale(DD5_T)*scale(MWMT_P) + (1|Trial/Block/Provenance), data = training)
  pair.mod2 <- lmer(log(height) ~ scale(MWMT_P) + scale(PAS_T) + scale(MWMT_P)*scale(PAS_T) + (1|Trial/Block/Provenance), data=training)
  diff.mod3 <- lmer(log(height) ~ scale(DD0_P) + scale(PAS_T) + scale(DD0_P)*scale(PAS_T) + (1|Trial/Block/Provenance), data=training)

  p.d3<-predict(dredge3,testing)-log(testing$height)
  p.d2<-predict(dredge2,testing)-log(testing$height)
  p.wh3<-predict(whittet.mod3,testing)-log(testing$height)
  p.pr2<-predict(pair.mod2,testing)-log(testing$height)
  p.df3<-predict(diff.mod3,testing)-log(testing$height)
...
#results<-read.csv(paste0(wd,'Scots_pine/cross-validation_results_interactions2.csv')) # PC
#results<-read.csv(paste0(wd,'data-raw/cross-validation_results_interactions2.csv')) # mac
#summary(results)
...
The results of the cross validation indicate that "dredge2" and "dredge3" have the highest average R2 marginal.
Both also showed the lowest average Mean Absolute Error (MAE).
These two models were therefore taken forwards to be tested within the DeltaTraitSDM framework.
The two models took the forms:
...
```{r dredge2, include=TRUE, echo=TRUE, warning=FALSE}

dredge2<-lmer(log(height) ~ scale(DD_18_T) + scale(NFFD_P) + scale(PAS_T) + scale(DD_18_T)*scale(NFFD_P) + scale(NFFD_P)*scale(PAS_T) + (1|Trial/Block/Provenance),data = sp.raw)

results <- report(dredge2, ci= 95)
...
```{r dredge3, include=TRUE, echo=TRUE, warning=FALSE}

dredge3<-lmer(log(height) ~ scale(DD_18_T) + scale(NFFD_P) + scale(TD_T) + scale(DD_18_T)*scale(NFFD_P) + scale(NFFD_P)*scale(TD_T)+ (1|Trial/Block/Provenance),data = sp.raw)

results <- report(dredge3, ci= 95)
...
By using separate rasters for the seed collection and planting years (Figure \@ref(fig:DTSDM2)) the future height prediction is constrained to a smaller area and predicted lower height overall.
