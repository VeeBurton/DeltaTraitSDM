...
# check climate data (are variables correct?)
library(sf)
climateSF<-st_as_sf(climate)
summary(climateSF$DD_18)
summary(climateSF$DD18)
 
climateSF %>% filter(lat>55) %>% 
   ggplot()+
   geom_point(aes(long,lat,fill=DD18))
...
values(ST_H) <- 125.9 # assign mean height to raster of same extent?
plot(ST_H)
rsmax <- stack(ST_H, MAP, FFP) #stack(ST_H, DD_18, NFFD)
names(rsmax)= c("Hraw","MAP_P","FFP_T")#c("Hraw","DD_18_T","NFFD_T")
names(rsmax)

#predict height
predH <- predict(rsmax, whittet.mod2, re.form=NA, type='response') #dredge1
predH<- log(predH)
